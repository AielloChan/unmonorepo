...
function getPkgJson(sourcePackageJson, packages, exclude = []) {
  ...
  while (jobs.length) {
    ...
    const curName = job.name;
    const curDeps = job.dependencies || {};
    for (const pkgName of Object.keys(curDeps)) {
      ...
      if (isMonorepoPkg) {
        ...
        const pkgVersion = curDeps[pkgName];
        if (pkgInfo) {
          // this module is already exists
          // show alert if version is different
          if (pkgInfo.version !== pkgVersion) {
            console.warn(
              `package ${pkgName} version is conflict, ${pkgInfo.version} in ${pkgInfo.source} vs ${pkgVersion} in ${curName}`
            );
          }
        } else {
